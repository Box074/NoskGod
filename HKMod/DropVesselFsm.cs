...
class DropVesselFsm : CSFsm<DropVesselFsm>
{
    ...
    private IEnumerator Spawn()
    {
        ...
        drop.AddComponent<DestroyOnNoskDie>();
        if (SpawnShade.Value)
        {
            drop.LocateMyFSM("Control").Fsm.GetState("Land").AppendFsmStateAction<InvokeAction>(new(a =>
            {
                if((Random.value >= 0.5f || NoskShade.GetShadeCount() > SpawnMaxCount.Value) && !returnSelf) return;
        ...
        drop.transform.SetScaleX(Random.Range(0, 100) <= 50 ? -1 : 1);
        drop.SetActive(true);
        drop.GetComponent<Rigidbody2D>().gravityScale = 1;
    }
    [FsmState]
